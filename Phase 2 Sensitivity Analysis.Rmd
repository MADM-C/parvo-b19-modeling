  ...
  with(
    ...
    v_transfusion <- c(n_it,
      ...
      df_output <- data.frame(Strategy =  v_names_str,
                              ...
                              IUTs_Total = total_it_sq + total_it_surv,
                              IUTs_SQ = total_it_sq,
                              IUTs_Surv = total_it_surv,
                              Transfusions_Averted = )
     return(df_output)})
}
...
df_params_owsa <- data.frame(pars = c("p_imm", "p_inf", "p_det_surv", "p_sfa", "p_und_it"),
                             # min parameter values
                             min  = c(0.25, 0, 0.05, 0.02, 0.6), 
...
# xa_cost <- c("Proportion Immune", "Infection Rate", "Detection Rate", "Severe Fetal Anemia Rate", "Transfusion Rate in Undetected")
# 
...
owsa_deaths <- run_owsa_det(params_range = df_params_owsa,  
                         ...
                         FUN        = run_tree, 
                         # outcomes
                         outcomes = c('B19_Deaths_Surv'),
                         # names of the strategies
                         strategies = v_names_str,
                         progress = FALSE)   
# tornado plot
# axis labels for tornado plots
#xa_deaths <- c("Infection Rate", "Severe Fetal Anemia Rate", "Proportion Immune", "Transfusion Rate in Undetected", "Detection Rate")
...
owsa_transfusions <- run_owsa_det(params_range = df_params_owsa,  
                         ...
                         FUN        = run_tree, 
                         # outcomes
                         outcomes = c('IUTs_Surv'),
                         # names of the strategies
                         strategies = v_names_str,
                         progress = FALSE)   
# tornado plot
# axis labels for tornado plots
#xa_transfusions <- c("Detection Rate", "Proportion Immune", "Infection Rate", "Severe Fetal Anemia Rate", "Transfusion Rate in Undetected")
...
owsa_averted <- run_owsa_det(params_range = df_params_owsa,  
                         ...
                         FUN        = run_tree, 
                         # outcomes
                         outcomes = c('Deaths_Averted', 'B19_Deaths_Surv'), 
                         # names of the strategies
                         strategies = v_names_str,
                         progress = FALSE)   
# tornado plot
# axis labels for tornado plots
torn_averted0 <- owsa_tornado(owsa = owsa_averted$outcome_val == 'Deaths_Averted') + 
...
xa_averted <- c("Undetected", "Proportion Immune", "Detection Rate", "Severe Fetal Anemia Rate", "Infection Rate")
...
# Mutate variable names in the owsa object!!
...
***
```{r, warning = FALSE}
# two way sensitivity analysis of detection rate and risk of developing severe fetal anemia
...
heat_deaths_ds <- ggplot(data = twsa_deaths, mapping = aes(x = p_det_surv,
  ...
  ggtitle("PVB19-Associated Deaths over Varying Detection Rates and Severe Fetal Anemia Rates") +
  scale_fill_viridis(option = "turbo", alpha = 0.55) +
  geom_text(aes(label = round(outcome_val, 0))) +
  guides(fill=guide_legend(title="Parvovirus Deaths"))

heat_deaths_ds
ggsave("heat_deaths_ds.png",
       plot = heat_deaths_ds)
...
heat_transfusions_ds <- ggplot(data = twsa_transfusions, mapping = aes(x = p_det_surv,
  ...
  xlab(label = 'Detection Rate') +
  ylab(label = 'Severe Fetal Anemia Rate') +
  ggtitle("Transfusions over Varying Detection Rates and Severe Fetal Anemia Rate") +
  scale_fill_viridis(option = "turbo", alpha = 0.55) +
  geom_text(aes(label = round(outcome_val, 0))) +
  guides(fill=guide_legend(title="Transfusions"))

heat_transfusions_ds
ggsave("heat_transfusions_ds.png",
        plot = heat_transfusions_ds)

twsa_averted <- data.matrix(twsa_averted)
heat_averted_ds <- ggplot(data = twsa_averted, mapping = aes(x = p_det_surv,
                                                  y = p_sfa,
                                                  fill = outcome_val)) +
  geom_tile() +
  xlab(label = 'Detection Rate') +
  ylab(label = 'Severe Fetal Anemia Rate') +
  ggtitle("Deaths Averted over Varying Detection Rates and Severe Fetal Anemia Rate") +
  scale_fill_viridis(option = "turbo", alpha = 0.55) +
  geom_text(aes(label = round(outcome_val, 0))) +
  guides(fill=guide_legend(title="Deaths Averted"))

heat_averted_ds
ggsave("heat_averted_ds.png",
       plot = heat_averted_ds)
```
***